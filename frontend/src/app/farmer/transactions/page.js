  ...
  return (
    <ProtectedRoute allowedRoles={['farmer']}>
      <DashboardLayout title="Farmer" navItems={navItems}>
        <h1 className="text-xl font-bold text-gray-900 mb-6">Transactions</h1>
              ...
              <Link key={tx._id} href={`/transactions/${tx._id}`} className="card hover:shadow-md transition-shadow block">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">Buyer: {tx.buyer?.name} · {formatDate(tx.createdAt)}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-primary-700">{formatCurrency(tx.totalAmount)}</p>
                    <p className="text-xs text-gray-400">{tx.quantity} units</p>
                  </div>
                </div>
